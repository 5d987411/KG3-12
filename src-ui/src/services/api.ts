...
async function kaspaAddressFromPublicKey(publicKeyBytes: Uint8Array, network: string = 'testnet-12'): Promise<string> {
...
export const greet = async (name: string): Promise<string> => {
  if (!isTauri || !invoke) {
    return `Hello, ${name}! (browser mode - REST API for testnet-12)`;
  }
